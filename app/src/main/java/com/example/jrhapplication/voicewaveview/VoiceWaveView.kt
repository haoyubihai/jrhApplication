...
class VoiceWaveView @JvmOverloads constructor(
    ...
    init {
        attrs?.let {
            ...
            lineWidth = typedArray.getDimension(R.styleable.VoiceWaveView_course_lineWidth, 20f)
            lineSpace = typedArray.getDimension(R.styleable.VoiceWaveView_course_lineSpace, 10f)
            duration = typedArray.getInt(R.styleable.VoiceWaveView_course_duration, 200).toLong()
            showGravity = typedArray.getInt(R.styleable.VoiceWaveView_android_gravity, Gravity.LEFT or Gravity.BOTTOM)
            lineColor = typedArray.getInt(R.styleable.VoiceWaveView_course_lineColor, Color.BLUE)
            val mode = typedArray.getInt(R.styleable.VoiceWaveView_course_waveMode, 0)
            when (mode) {
                0 -> waveMode = WaveMode.UP_DOWN
                1 -> waveMode = WaveMode.LEFT_RIGHT
            }

            val lType = typedArray.getInt(R.styleable.VoiceWaveView_course_lineType, 0)
            when (lType) {
    ...
    fun changeBody(num: Int){
        bodyWaveList.isNotEmpty()?.let {
            lastChangeIndex++
            lastChangeIndex %= bodyWaveList.size
            bodyWaveList[lastChangeIndex] = num
        }
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (i in waveList.indices) {
            ...
            var lineHeight = waveList[i] / 0.0
            //注意需要对线条宽度修正
            lineHeight = if (lineHeight+2*lineWidth>measuredHeight) measuredHeight.toDouble()-2*lineWidth else lineHeight
    ...
    fun clearBody() = bodyWaveList.clear()
}